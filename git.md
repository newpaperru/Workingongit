...
Используется для того, чтобы отправить файлы с GitHub репозитория на Ваш компьютер, который до этого был __привязан и создан__. 

`Внимание! Для того, чтобы все отправилось на Ваш репозиторий на GitHub, нужно "Подружить" Ваш локальный репозиторий с репозиторием на GitHub. (см. пункт знакомство с GitHub)`

`Внимание! Не путайте понятие Clone и Pull. Clone используется когда Вам нужно чей-то репозиторий СКАЧАТЬ на Ваш компьютер, а Pull используется, чтобы ОБНОВИТЬ файлы, которые были изменены на GitHub, но на Вашем локальном файле не были обновлены.`